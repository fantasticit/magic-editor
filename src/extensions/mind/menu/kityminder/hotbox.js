      ...
      function HotBoxState(hotBox, stateName) {
        ...
        function layout() {
          ...
          function layoutRing(radius) {
            ...
            $ringShape.style.marginLeft = $ringShape.style.marginTop =
              -radius + "px";
            $ringShape.style.width = $ringShape.style.height =
              radius + radius + "px";
          ...
          function layoutTop(radius) {
            ...
            buttons.top.forEach(function(topButton) {
              ...
              topButton.indexedPosition = [
                xOffset + $button.offsetLeft + $button.clientWidth / 2,
                yOffset
              ];
            });
          }
        ...
        function defaultButtonRender(format, option) {
          return format(
            '<span class="label">{label}</span><span class="key">{key}</span>',
            {
              label: option.label,
        ...
        this.handleKeyEvent = function(e) {
          ...
          if (e.keydown || (hotBox.isIME && e.keyup)) {
            ...
            e.keyup &&
            (stateActived || !hotBox.hintDeactiveMainState)
          ) {
            if (pressedButton) {
              if (
                (e.isKey("space") && selectedButton == pressedButton) ||
                e.isKey(pressedButton.key)
              ) {
