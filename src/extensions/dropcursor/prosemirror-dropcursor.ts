...
class DropCursorView {
  ...
  timeout: ReturnType<typeof setTimeout> = setTimeout(() => "", 0);
  ...
  updateOverlay() {
    ...
    if (!$pos.parent.inlineContent) {
      let before = $pos.nodeBefore,
        after = $pos.nodeAfter;
      if (before || after) {
        let dom$1 = this.editorView.nodeDOM(
          this.cursorPos! - (before ? before.nodeSize : 0)
        ) as HTMLElement;

        if (dom$1) {
          while (dom$1 && dom$1.nodeType === 3 && dom$1.parentElement) {
            dom$1 = dom$1.parentElement;
          }

          let nodeRect = dom$1.getBoundingClientRect();

          let top = before ? nodeRect.bottom : nodeRect.top;

          if (before && after) {
            let dom = this.editorView.nodeDOM(this.cursorPos!) as HTMLElement;

            if (dom) {
              while (dom && dom.nodeType === 3 && dom.parentElement) {
