...
export const defaultYSyncPluginOpts = {
...
export const ySyncPlugin = (
  ...
  const plugin = new Plugin({
    ...
    view: view => {
      const binding = new ProsemirrorBinding(yXmlFragment, view, blockIds);
...
export class ProsemirrorBinding {
   ...
   */
  constructor(yXmlFragment, prosemirrorView, blockIds) {
    this.type = yXmlFragment;
    this.prosemirrorView = prosemirrorView;
    this.blockIds = navigator.userAgent.includes("Chrome")
      ? blockIds
      : [1, 2, 3];
  ...
  _forceRerender() {
    this.mapping = new Map();
    this.mux(() => {
      if (!navigator.userAgent.includes("Chrome") && window.blockIds) {
        console.log("已经载入配置", window.blockIds);
        this.blockIds = window.blockIds;
      }
      const fragmentContent = this.type
        .toArray()
        .map(t => {
          console.log("render force", t);

          if (this.blockIds && this.blockIds.length) {
            const blockId = t.getAttribute("blockId");
            const shouldShow = blockId ? this.blockIds.includes(blockId) : true;
        ...
        const fragmentContent = Y.typeListToArraySnapshot(
          ...
          .map(t => {
            if (
              ...
              console.log("render snop", t);
              if (this.blockIds && this.blockIds.length) {
                const blockId = t.getAttribute("blockId");
                const shouldShow = blockId
                  ? this.blockIds.includes(blockId)
                  : true;
                if (!shouldShow) return null;
              }
  ...
  _typeChanged(events, transaction) {
    ...
    this.mux(() => {
      ...
      const fragmentContent = this.type
        .toArray()
        .map(t => {
          console.log("render change", t);

          if (this.blockIds && this.blockIds.length) {
            const blockId = t.getAttribute("blockId");
            let shouldShow = blockId ? this.blockIds.includes(blockId) : true;

            if (!shouldShow) {
              let parent = t.parent;

              if (parent.getAttribute) {
                shouldShow = this.blockIds.includes(
                  parent.getAttribute("blockId")
                );
              } else if (parent.firstChild) {
                shouldShow = this.blockIds.includes(
                  parent.firstChild.getAttribute("blockId")
                );
              }
              console.log("父节点 测试", parent);
            }
