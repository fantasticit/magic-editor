...
export const createDecorations = (
  ...
  createCursor,
  createSelection,
  blockEditCursorCache: Map<string, Record<string, unknown>>
) => {
  ...
  awareness.getStates().forEach((aw, clientId) => {
    ...
    if (aw.cursor !== null) {
      if (blockEditCursorCache.has(clientId)) {
        blockEditCursorCache.delete(clientId);
      }
    } else {
      aw.cursor = blockEditCursorCache.get(clientId);
    }
    ...
    if (aw.cursor != null) {
      ...
      if (anchor !== null && head !== null) {
        ...
        const maybeNode = findNodeAt(state, from, to);

        if (
          maybeNode &&
          // @ts-ignore
          maybeNode?.node?.isAtom &&
          // @ts-ignore
          !maybeNode?.node?.isTextBlock &&
          // @ts-ignore
          !maybeNode.node.isInline
        ) {
          blockEditCursorCache.set(clientId, {
            anchor: aw.cursor.anchor,
            head: aw.cursor.head
          });
          decorations.push(
            Decoration.node(maybeNode.pos, maybeNode.pos + 1, {
              nodeName: "div",
...
export const yCursorPlugin = (
  ...
  const blockEditCursorCache = new Map();
